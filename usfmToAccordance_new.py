...
Usage:
    python usfmToAccordance_new.py test1.usfm > test1.acc
    python usfmToAccordance_new.py --no-para --no-tc test3.usfm > test3.acc
    python usfmToAccordance_new.py *.usfm > combined.acc
"""
...
# Ensure stdout uses UTF-8 encoding (important for Windows)
...
def main(para, tc, debug, files):
    ...
    Processes one or more USFM files and outputs the combined result to stdout.
    Error messages and warnings are sent to stderr.
    """
